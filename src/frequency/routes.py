...
@router.get("/")
async def get_all_frequency_periods(db: Session = Depends(get_db), token = Depends(get_current_token)):
    try:
...
@router.get("/currents")
async def get_all_current_frequency_periods(db: Session = Depends(get_db), token = Depends(get_current_token)):
    try:
...
async def get_all_frequency_periods_for_sensor(sensor_id: int, db: Session = Depends(get_db), token = Depends(get_current_token)):
    try:
...
async def get_current_frequency_period_for_sensor(sensor_id: int, db: Session = Depends(get_db), token = Depends(get_current_token)):
    try:
